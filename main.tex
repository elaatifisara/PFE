...
\addto\captionsfrench{\renewcommand{\listfigurename}{Liste de figure}}
\usepackage{graphicx}
\usepackage{geometry}
\usepackage{fancyhdr}
\usepackage{tikz}
...
\geometry{top=2.0cm, bottom=2.5cm, left=2.5cm, right=2.5cm, headsep=12pt}
...
\setlength{\headheight}{45pt} % Réduit pour rapprocher le texte du haut
...
\begin{center}
 ...
 \begin{minipage}{\textwidth}
    ...
    \begin{minipage}{0.45\textwidth}
        \raggedleft
        \textbf{Encadré par  :}\\
        Pr. OUNACER Soumaya (Encadrante)  \\ Mme ABBOUR Fatima Zahra (Co-encadrante) 
    \end{minipage}
\vspace{1.5cm}

\begin{minipage}{\textwidth}
\begin{flushleft}
\textbf{Soutenu le 20 Juin 2026 devant le jury composé de :}\\[0.2cm]
\vspace{0.5cm}
\begin{tabular}{l l}
Pr. Azzouazi Mohamed (Président)\\
Pr. DAIF Abderrahmane (Examinateur)    \\
Pr. OUNACER Soumaya(Encadrante)\\
Mme. ABBOUR Fatima Zahra  (Co-encadrante) \\
\end{tabular}
\end{flushleft}
\end{minipage}
\end{center}
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\raisebox{-6pt}{\includegraphics[height=1.5cm]{logo fsbm.png}}}
\fancyhead[R]{\raisebox{-6pt}{\includegraphics[height=1.5cm]{logo dep.png}}}
\raggedbottom
...
\justifying
Avant de développer une plateforme touristique unifiée, il est essentiel d'analyser les solutions existantes afin d'identifier leurs points forts, leurs limites et les opportunités d'amélioration. Le marché du tourisme en ligne est aujourd'hui dominé par plusieurs plateformes spécialisées qui couvrent chacune un segment précis du parcours voyageur (hébergement, transport, activités ou avis), mais sans offrir une véritable intégration globale \cite{buhalis2008progress, gretzel2015smart}.


Dans le cadre de cette étude, une analyse comparative a été réalisée sur plusieurs acteurs majeurs du secteur, notamment Google Travel \cite{googletravel}, Booking.com \cite{booking}, Airbnb \cite{airbnb}, TripAdvisor \cite{tripadvisor2024}, Expedia \cite{expedia2024}, GetYourGuide \cite{getyourguide2024} et Uber \cite{uber2024}. L'objectif est d'évaluer leurs fonctionnalités, leur niveau d'intégration, leur couverture du marché marocain ainsi que leur capacité à répondre aux besoins des utilisateurs.
...
\caption{Comparatif des plateformes touristiques existantes}
\label{tab:comparatif_plateformes}
...
\textbf{Plateforme} &
\textbf{Hébergement} &
\textbf{Transport aéroport} &
\textbf{Lieux touristiques} &
\textbf{Avis utilisateurs} &
\textbf{Multilingue FR/EN} &
\textbf{Paiement local Maroc} &
\textbf{Transparence prix} \\
\hline
...
\hline
GetYourGuide & Absent & Non & Activités seules & Oui & Oui & Non & Sans objet \\
\hline
...
\end{table}
\vspace{0.3em}
{\footnotesize Source : élaboré par les auteurs à partir des sites officiels
des plateformes \cite{googletravel, booking, airbnb, tripadvisor2024, expedia2024, getyourguide2024, uber2024}.}
...
Ensuite, les plateformes actuelles prennent peu en compte les spécificités du marché marocain, notamment en matière de moyens de paiement, de couverture géographique et d'adaptation aux services locaux \cite{buhalis2008progress}.
...
D'un point de vue technique, ces limites viennent surtout du fait que chaque plateforme fonctionne de façon isolée. Chacune possède sa propre base de données et ne partage pas ses informations avec les autres. Il est donc très difficile de réunir tous ces services au même endroit : l'utilisateur doit créer un compte et se connecter sur chaque plateforme séparément, et aucune d'elles ne regroupe l'ensemble de son voyage.
Cette séparation pose aussi un problème pour les paiements. Comme chaque service est réservé de son côté, l'utilisateur doit effectuer plusieurs paiements distincts, sans lien entre eux. Il peut alors arriver qu'un paiement soit réalisé sans que la réservation correspondante soit confirmée, car aucun système ne relie les deux.
Enfin, ces plateformes sont pensées pour le marché mondial et non pour le contexte marocain. Elles utilisent souvent des services de cartographie payants, soumis à des limites d'utilisation, et ne proposent pas les moyens de paiement ni les langues adaptés aux utilisateurs locaux. De plus, leurs prix manquent de transparence : ils incluent des commissions variables, difficiles à vérifier pour le voyageur [1, 2].
...
Contrairement à plusieurs plateformes spécialisées, TravEasy permet de centraliser la recherche et la réservation d'hébergements touristiques au Maroc au sein d'un même environnement numérique. Grâce à un système de recherche multicritère et à une interface intuitive,  l'utilisateur peut comparer plusieurs établissements et effectuer sa réservation directement sur la plateforme. Un lien vers le site officiel de l'établissement est par ailleurs proposé à titre informatif.

Les choix techniques de TravEasy répondent directement à ces limites. La plateforme s'appuie sur une seule base de données relationnelle, gérée avec MySQL [15], qui réunit au même endroit les utilisateurs, les hébergements, les transports, les paiements et les avis. Comme toutes ces informations sont liées entre elles, une réservation et son paiement sont enregistrés ensemble et restent cohérents — ce que la combinaison de plusieurs services extérieurs ne permet pas.
La connexion des utilisateurs est centralisée grâce à Firebase [21] : l'utilisateur ne se connecte qu'une seule fois pour accéder à tous les services, et la sécurité est gérée au même endroit. L'application est par ailleurs construite d'un seul tenant (voir section 4.3), ce qui rend les échanges entre ses différentes parties plus rapides et le développement plus simple, tout en restant organisée pour pouvoir évoluer plus tard [10].
Le site et le serveur communiquent à travers une API REST [20], qui sépare clairement la partie visible par l'utilisateur de la partie qui traite les données ; cette organisation facilite l'évolution du système. La partie visible, développée avec React [17] et Next.js, offre un affichage rapide et un meilleur référencement, utile pour une plateforme touristique destinée à être trouvée facilement en ligne.
Pour la cartographie, le choix d'OpenStreetMap [22], gratuit et libre, évite les coûts et les limites des cartes payantes tout en gardant la maîtrise des données de localisation. Enfin, la transparence des prix repose sur un calcul effectué directement par le serveur à partir de critères clairs (distance, type de véhicule, horaires, saison) : le prix obtenu est donc vérifiable, contrairement aux tarifs peu clairs des autres plateformes.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{di_previ.jpeg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{di_reel.jpeg}
...
La démarche de conception adoptée s'appuie sur les bonnes pratiques du génie logiciel
\cite{sommerville2016}, et la modélisation du système repose sur la notation standard UML
\cite{booch2005}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{diagramme_de_classes_TravEasy (1) copy.png}
    \caption{Diagramme de classe  }
    \label{fig: Diagramme de classe }
\end{figure}

Ce diagramme constitue le socle de la base de données : chaque classe correspond à une entité réelle de la plateforme (utilisateur, hébergement, réservation, paiement, avis) et les liens entre elles traduisent les relations qui seront ensuite transformées en tables et en clés étrangères. Son apport est de garantir dès la conception la cohérence des données et de servir de référence directe pour la création du schéma relationnel présenté au chapitre 4.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{usecase ppt.jpeg}
...
Ce diagramme illustre les principales fonctionnalités disponibles au sein de la plateforme, notamment l'authentification, la gestion des réservations, la consultation des contenus touristiques et les opérations de gestion associées. Son apport est de relier chaque acteur (visiteur, utilisateur, administrateur) aux fonctionnalités qui le concernent, et donc de délimiter clairement le périmètre du système. Il sert ainsi de point de départ à la définition des besoins fonctionnels et aux scénarios détaillés modélisés ensuite par les diagrammes de séquence.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{activite.jpeg}
    \caption{Diagramme d'activité -Authentification   }
    \label{Diagramme d'activité -Authentification }
\end{figure}
Le diagramme précédent illustre les différentes étapes nécessaires au processus d'authentification, depuis la saisie des informations d'identification jusqu'à la validation des accès utilisateur, garantissant ainsi la sécurité du système. Son apport est de décrire la logique du processus, notamment la vérification des identifiants et la gestion des cas d'échec, avant son implémentation. Cela permet d'anticiper les différents chemins possibles et de sécuriser l'accès à la plateforme.
...
\begin{spacing}{1.5}
 \begin{figure}[H]
     \centering
     \includegraphics[width=1\textwidth]{diag_auth.png} 
     \caption{Diagramme de séquence -- Authentification}
     \label{fig:seq_auth}
 \end{figure}

 Ce diagramme précise, dans l'ordre chronologique, les échanges entre l'utilisateur, l'interface, le serveur et le service d'authentification lors de la connexion. Son apport est de montrer concrètement comment ces composants collaborent dans le temps et de vérifier que chaque étape (saisie, vérification, réponse) est bien prise en charge avant le développement.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.1\textwidth]{filtre seq.png}
    \caption{Diagramme de séquence -- Recherche de lieux touristiques }
    \label{fig:seq_catalog}
\end{figure}

Ce diagramme détaille le déroulement d'une recherche filtrée : la requête de l'utilisateur, son traitement par le serveur et la restitution des résultats depuis la base de données. Son apport est de valider, au niveau de la conception, le bon enchaînement des échanges nécessaires à la recherche multicritère.
...
\begin{spacing}{1.5}
    \begin{figure}[H]
        \centering
        \includegraphics[width=1\textwidth]{diagramme-reservation-transport.png} 
        \caption{Diagramme de séquence -- Réservation de transport aéroport}
        \label{fig:seq_auth}
    \end{figure}

    Ce diagramme modélise les interactions successives menant à la réservation d'un transport aéroportuaire, y compris le calcul du prix et l'étape de paiement. Son apport est de clarifier, avant l'implémentation, la coordination entre la réservation et le paiement, qui doivent rester cohérents.
...
\subsection{Gestion des réservations d’hébergements}
\vspace{0.5cm}
\begin{spacing}{1.5}
\justifying
 
%Ce diagramme de séquence illustre les interactions entre l’utilisateur, l’interface, le
%serveur et les services externes lors du processus de réservation. Il montre les différentes
%étapes du traitement de la demande, depuis la saisie des informations jusqu’à la confir-
%mation de l’opération. Son apport est de représenter l'ensemble du flux de réservation d'un hébergement, de la saisie jusqu'à la confirmation, et de mettre en évidence les moments où interviennent le serveur et les services externes — ce qui guide directement la réalisation de ce module.
...
\begin{spacing}{1.5}
    \begin{figure}[H]
        \centering
        \includegraphics[width=1\textwidth]{diag_seq_ges_res.png} 
        \caption{Diagramme de séquence -- Gestion des réservations d’hébergements}
        \label{fig:seq_auth}
    \end{figure}
...
Visual Studio Code \cite{vscode} & Développement \\ \hline
Git / GitHub \cite{git, github}  & Gestion des versions \\ \hline
Postman  & Test des API \\ \hline
MySQL \cite{mysql} & Base de données \\ \hline
Draw.io / StarUML \cite{drawio} & Diagrammes UML \\ \hline
\end{StdTable}
...
Firebase \cite{firebase2024} avec SSO google a été choisi pour la gestion de l'authentification en raison de sa simplicité d'intégration, de sa sécurité renforcée.  
Open Street Map \cite{openstreetmap2024} a été choisie pour sa précision, sa couverture géographique mondiale et ses fonctionnalités avancées de géolocalisation et de cartographie. Pour les paiements en ligne, Stripe \cite{stripeapi} et PayPal \cite{paypal2024} ont été retenus en raison de leur fiabilité, de leur niveau élevé de sécurité et de leur large adoption dans le commerce électronique. Le recours à ces solutions permet de bénéficier de services éprouvés et régulièrement mis à jour, évitant ainsi le développement de fonctionnalités complexes tout en garantissant une meilleure expérience utilisateur.
\end{spacing}
...
Cette page présente les principales fonctionnalités de la plateforme et permet aux visiteurs de découvrir les services disponibles sans authentification.
...
Cette section met en avant les destinations, hébergements et services touristiques proposés par la plateforme.
...
Après authentification, l'utilisateur accède à un espace personnalisé lui permettant de gérer ses réservations et préférences.
...
Cette capture illustre la version anglaise de l'interface afin de répondre aux besoins des utilisateurs internationaux.
...
Cette capture représente la suite de la page d'accueil en version anglaise, en présentant des informations complémentaires et les différents services touristiques proposés par la plateforme.
\end{spacing}
...
Ce module intègre également un mécanisme de gestion des rôles permettant de définir différents niveaux d'accès et d'autorisation selon le profil de l'utilisateur (administrateur, utilisateur standard, etc.)
...
Cette page regroupe l'ensemble des destinations et activités touristiques disponibles. Elle met à disposition de l'utilisateur des outils de recherche multicritère, de filtrage et de tri (par note ou par prix) afin de faciliter la comparaison et la sélection des offres.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{catalogue_exemple copy.png}
...
\begin{spacing}{1.5}
\justifying
Le module d'hébergement a été développé afin de permettre aux utilisateurs de rechercher, consulter et réserver différents types d'établissements touristiques disponibles sur la plateforme, notamment les hôtels, les riads, les auberges et les maisons d'hôtes. Il vise à faciliter l'accès aux informations relatives aux hébergements et à offrir une expérience de réservation simple et centralisée.

Parmi les principales fonctionnalités proposées figurent la consultation des hébergements disponibles, la recherche par nom ou par ville, ainsi que plusieurs options de filtrage permettant d'affiner les résultats selon différents critères tels que le type d'hébergement, la localisation, le nombre d'étoiles ou encore la gamme de prix. Afin d'améliorer l'expérience utilisateur et de faciliter la navigation, les résultats sont organisés à l'aide d'un système de pagination.

Le module permet également d'accéder aux détails complets d'un établissement, incluant sa description, ses équipements, ses tarifs et les informations utiles à la réservation. Contrairement à une simple redirection vers un site externe, TravEasy offre la possibilité d'effectuer la réservation directement sur la plateforme, garantissant ainsi un parcours utilisateur plus fluide, rapide et sécurisé. Cette approche contribue à simplifier la recherche et la réservation d'hébergements au sein d'un environnement unique.

Depuis la liste des hébergements, chaque établissement propose deux actions : un bouton « Détails », qui ouvre la page de détail de l'hébergement au sein de la plateforme et permet d'effectuer la réservation ; et un bouton « Voir le site officiel », qui redirige, à titre complémentaire, vers le site officiel de l'établissement.
...
L'utilisateur peut rechercher des hébergements selon différents critères tels que la localisation, le prix ou la catégorie.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{capture_mamounia_v2 copy.png}
\caption{Exemple d'hébergement}
\label{fig:Exemple d'hébergement}
\end{figure}

Cette figure présente la fiche détaillée d'un hébergement, incluant ses caractéristiques principales, ses équipements ainsi que les informations utiles à la réservation. L'utilisateur peut y consulter l'ensemble des détails nécessaires avant de procéder directement à la réservation sur la plateforme.
...
Cette interface permet à l'utilisateur d'estimer et de réserver un transport vers ou depuis un aéroport. Après sélection du point de départ et de la destination, le système calcule automatiquement le tarif selon la formule de tarification dynamique présentée précédemment.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{capture_transport_exemple copy.png}
...
Cette page permet à l'utilisateur de finaliser sa réservation à travers un paiement sécurisé. Elle s'appuie sur des passerelles fiables (Stripe, PayPal) et procède à la validation de la transaction avant la confirmation définitive de la réservation.
...
Cette interface permet de consulter les avis et les notes attribués par les utilisateurs aux différents services proposés.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{capture_espace_profile copy.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{capture_resrvartion copy 2.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{capture_payments copy.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{capture_favoris copy.png}
    \caption{Onglet des favoris dans l'espace profil}
    \label{fig:Onglet favoris profil}
    \end{figure}

    Cet onglet permet à l'utilisateur de regrouper et de gérer ses éléments favoris (hébergements, activités, locations), lui offrant un accès rapide à ses sélections et contribuant ainsi à une expérience plus personnalisée et adaptée à ses centres d'intérêt.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{capture_preference copy.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{dash_admin.png.jpeg}
\caption{Tableau de bord administrateur}
\label{fig:Tableau de bord administrateur}
\end{figure}

Ce tableau de bord offre à l'administrateur une vue d'ensemble de l'activité de la plateforme. Il permet de superviser les utilisateurs, les réservations et les contenus, et présente des statistiques clés (nombre d'utilisateurs, de réservations et d'avis) facilitant la prise de décision.
...
Cette section regroupe les réponses aux questions les plus fréquentes (création de compte, réservations, paiement, conditions d'utilisation), permettant aux utilisateurs de résoudre leurs difficultés de manière rapide et autonome.
...
Ce formulaire permet aux utilisateurs de soumettre une demande d'assistance, de signaler un problème ou d'envoyer une suggestion. Les messages sont transmis à l'équipe d'administration via un service de courrier électronique sécurisé .
...
TH06 & Bouton « Détails » & Ouverture de la page de détail de l'hébergement sur la plateforme, permettant la réservation & Conforme \\ \hline
TH07 & Bouton « Voir le site officiel » & Redirection vers le site officiel de l'établissement & Conforme \\ \hline
\end{StdTable}
\textbf{Analyse des résultats}

Les tests réalisés ont permis de confirmer le bon fonctionnement de l'ensemble des fonctionnalités du module d'hébergement. Les recherches et les différents filtres appliqués retournent des résultats cohérents avec les critères sélectionnés par l'utilisateur. Le système de pagination facilite la navigation entre les résultats. La consultation des détails ouvre la page de réservation interne à la plateforme, tandis qu'un lien complémentaire permet, le cas échéant, d'accéder au site officiel de l'établissement.
...
\section{Synthèse quantitative des tests}
\vspace{0.5cm}
\begin{spacing}{1.5}
\justifying

Afin de consolider la validation de la plateforme, l'ensemble des tests réalisés est synthétisé sous forme d'indicateurs chiffrés. Le tableau suivant regroupe, par catégorie, le nombre de cas testés, le nombre de cas conformes et le taux de réussite correspondant.
...
\begin{table}[h]
    ...
    \hline
    \textbf{Catégorie de test} & \textbf{Cas testés} & \textbf{Conformes} & \textbf{Taux de réussite} \\ \hline
    Tests fonctionnels   & 25 & 25 & 100\% \\ \hline
    Tests d'intégration  & 5  & 5  & 100\% \\ \hline
    Tests de sécurité    & 6  & 6  & 100\% \\ \hline
    \textbf{Total}       & \textbf{36} & \textbf{36} & \textbf{100\%} \\ \hline
    \end{tabular}
    \caption{Synthèse quantitative des tests réalisés}
    \end{table}

    Le tableau ci-dessus montre que la totalité des 36 cas de test exécutés a été validée, soit un taux de réussite global de 100 %. Aucune anomalie bloquante n'a été relevée, que ce soit au niveau des fonctionnalités individuelles, des interactions entre modules ou des mécanismes de sécurité. Ces indicateurs, complétés par des temps de réponse inférieurs à deux secondes, confirment la fiabilité, la stabilité et les bonnes performances de la plateforme TravEasy.
...
\begin{spacing}{1.5}
\justifying
Au terme de ce projet de fin d'études, nous avons pu concevoir et développer TravEasy, une plateforme web touristique unifiée ayant pour objectif de faciliter l'accès aux principaux services touristiques au Maroc à travers une interface unique, moderne et intuitive. Ce projet nous a permis d'appliquer les connaissances acquises tout au long de notre formation en Développement Informatique, notamment dans les domaines du développement web, de la modélisation UML, de la conception de bases de données, de la sécurité informatique et de la gestion de projets logiciels.

\textbf{Objectifs atteints.} L'ensemble des objectifs définis lors de la phase d'analyse a été atteint. Les sept objectifs initiaux --- gestion des utilisateurs, recherche multicritère, réservation de transport, paiement sécurisé, gestion des avis, interface multilingue et tableau de bord administrateur --- ont été validés. Les 36 cas de test réalisés ont abouti à un taux de réussite de 100\,\%, avec des temps de réponse inférieurs à deux secondes, confirmant la stabilité, la sécurité et les bonnes performances de la solution.

\textbf{Principales réalisations.} La plateforme intègre la consultation des destinations touristiques, la recherche, le filtrage et la réservation directe d'hébergements, la réservation de transports aéroportuaires, la location de véhicules, le paiement en ligne sécurisé, la gestion des avis et des notations, ainsi qu'une interface bilingue français–anglais destinée à répondre aux attentes d'une clientèle nationale et internationale. Sur le plan technique, l'utilisation de technologies modernes telles que React.js, Node.js, Express.js et MySQL, associée à une architecture unifiée, a permis de développer une application performante, évolutive et sécurisée, répondant aux limites des solutions existantes identifiées lors de l'étude préalable.

\textbf{Perspectives futures.} Plusieurs perspectives d'amélioration peuvent être envisagées pour enrichir davantage la plateforme. Parmi celles-ci figurent le développement d'une application mobile, l'ajout de nouvelles langues, l'intégration d'un système de paiement échelonné, ainsi que la mise en place de mécanismes de recommandation intelligente permettant de personnaliser davantage l'expérience utilisateur.
