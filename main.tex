...
\begin{minipage}{\textwidth}
\begin{flushleft}
\textbf{Soutenu le    Juin 2026 devant le jury composé de :}\\[0.2cm]
\vspace{0.5cm}
\begin{tabular}{l l}
Pr.       \\
Pr.  \\
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\raisebox{-10pt}{\includegraphics[height=1.5cm]{logo fsbm.png}}}
\fancyhead[R]{\raisebox{-10pt}{\includegraphics[height=1.5cm]{logo dep.png}}}
...
\vspace{2cm}\textbf{Mots-clés :} Platforme web, Bilinguisme, Destinations touristiques, Maroc, Plateforme unifiée, Réservation, Tourisme.
% ------------------ ABSTRACT ------------------
\newpage
\vspace*{0.5cm}
\begin{center}
...
\textbf{Keywords:} Bilingualism, Booking, Destination catalog, Morocco, Tourism, Unified platform, Web application.
% ------------------ TABLE DES MATIÈRES ------------------
\newpage
\tableofcontents
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{img fac.png}
    \caption{Image de la Faculté des Sciences Ben M’Sick }
    \label{fig:Zones d'activité du groupe OCP au Maroc}
\end{figure}
\end{spacing}
...
\begin{itemize}
    ...
    \item Proposer un moteur de recommandation d'hébergements adapté aux besoins et au budget des voyageurs.
...
\renewcommand{\arraystretch}{1.3} % Augmente la hauteur des lignes de 30%
\begin{table}[h]
\centering
\caption{Comparatif des plateformes}
...
\centering
\caption{Comparatif des plateformes}
% \textwidth force le tableau à prendre exactement la largeur de la page.
% Le "X" (en majuscule) crée des colonnes qui s'adaptent et reviennent à la ligne automatiquement.
\begin{tabularx}{\textwidth}{|X|X|X|X|X|X|}
\hline
Plateforme & Transport aéroport & Lieux \newline touristiques & Hébergement & Bilingue FR/EN & Paiement \\
\hline
Google Travel & Non & Oui \newline (agrégation) & Oui \newline (agrégation) & Partiel & Non \\
\hline
Booking.com & Partiel & Non & Très étendu & Oui & Partiel \\
\hline
Airbnb & Non & Partiellement & Oui \newline (particuliers) & Oui & Non \\
\hline
...
\begin{itemize}
    ...
    \item Module d'hébergement : Consultation des hôtels, riads, auberges et maisons d'hôtes ,recherche par nom ou ville ,Redirection vers le site officiel de réservation
    ...
    \item Ces modules permettent de couvrir l’ensemble du parcours utilisateur de manière centralisée et cohérente
\end{itemize}
\end{spacing}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.50\textwidth]{Capture d’écran_30-5-2026_15616_.jpeg}
    \caption{ }
    \label{fig: }
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.50\textwidth]{ChatGPT Image 30 mai 2026, 11_32_33.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.50\textwidth]{Diagramme sans nom.drawio (1).png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{classe22.png}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth]{usecase111.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{rapp fig/activite authentification.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{rapp fig/sequence gestion de reser.png}
    \caption{Diagramme de séquence -Gestion de réservation   }
    \label{Diagramme de séquence -Gestion de réservation }
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{rapp fig/sequence reservation.png}
    \caption{ Diagramme de séquence-Gestion des utilisateurs   }
    \label{ Diagramme de séquence-Gestion des utilisateurs   }
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{rapp fig/sequence lieux touri.png}
    \caption{ Diagramme de séquence-Recherche de lieux touristiques     }
    \label{ Diagramme de séquence-Recherche de lieux touristiques   }
\end{figure}
\end{spacing}
\end{spacing}
...
\begin{figure}[H]
    ...
    \label{ Diagramme de séquence-Réservation de transport  }
\end{figure}
...
MySQL / PostgreSQL & Base de données \\ \hline
...
\begin{table}[H]
\centering
\caption{Table Point d'intérêt}
\label{tab:point_interet}
\begin{tabular}{|l|l|}
\hline
\textbf{Champ} & \textbf{Type} \\ \hline
id & INT \\ \hline
...
\begin{table}[H]
\centering
\caption{Table Estimation de trajet}
\label{tab:estimation_trajet}
\begin{tabular}{|l|l|}
\hline
\textbf{Champ} & \textbf{Type} \\ \hline
...
\begin{spacing}{1.5}
\justifying
Le module de réservation de transport permet aux utilisateurs de réserver facilement un moyen de transport pour leurs déplacements. Lors du processus de réservation, l’utilisateur sélectionne d’abord son \textbf{point de départ} ainsi que sa \textbf{destination}. À partir de ces informations, le système effectue un \textbf{calcul automatique du tarif} en tenant compte de plusieurs paramètres liés au trajet. Une fois le montant calculé et les informations vérifiées, l’utilisateur peut procéder à la \textbf{confirmation de la réservation}.
...
\begin{equation}
\text{Prix} = \text{TarifBase} + (\text{Distance} \times \text{Coefficient}) + \text{MajorationNuit} + \text{MajorationWeekend} + \text{MajorationSaison}
\end{equation}
où :
\begin{itemize}
...
\begin{table}[H]
\centering
\caption{API de Transport}
\label{tab:api_transport}
\begin{tabular}{|l|l|p{7cm}|}
\hline
\textbf{Méthode} & \textbf{Endpoint} & \textbf{Description} \\ \hline
...
\begin{table}[H]
\centering
\caption{API de Paiement}
\label{tab:api_paiement}
\begin{tabular}{|l|l|p{7cm}|}
\hline
\textbf{Méthode} & \textbf{Endpoint} & \textbf{Description} \\ \hline
...
\begin{table}[H]
\centering
\caption{API des Lieux Touristiques}
\label{tab:api_places}
...
\begin{itemize}
    \item \textbf{Authentification par JWT}
\end{itemize}
L’authentification des utilisateurs repose sur l’utilisation des \textbf{JSON Web Tokens (JWT)}. Après une connexion réussie, un jeton sécurisé est généré et transmis au client. Ce mécanisme permet de vérifier l’identité de l’utilisateur lors de chaque requête tout en limitant les risques d’accès non autorisés aux ressources protégées.
\begin{itemize}
    \item \textbf{Hashage des mots de passe}
\end{itemize}
Les mots de passe des utilisateurs ne sont jamais stockés en clair dans la base de données. Ils sont chiffrés à l’aide de la bibliothèque \textbf{bcrypt}, qui applique une fonction de hachage robuste avant leur enregistrement. Cette approche renforce la protection des comptes utilisateurs en cas de compromission de la base de données.
\begin{itemize}
...
\begin{table}[H]
\centering
\caption{Tests Fonctionnels - Inscription}
\label{tab:test_inscription}
...
\begin{table}[H]
\centering
\caption{Tests Fonctionnels - Connexion}
\label{tab:test_connexion}
...
\begin{table}[H]
\centering
\caption{Tests Fonctionnels - Recherche de lieux touristiques}
\label{tab:test_module_connexion}
\begin{tabular}{|l|p{4cm}|p{5cm}|l|}
\hline
\textbf{ID} & \textbf{Action} & \textbf{Résultat attendu} & \textbf{Résultat obtenu} \\ \hline
...
\centering
\caption{Tests Fonctionnels - Recherche de lieux touristiques}
\label{tab:test_recherche_lieux}
...
\begin{table}[H]
\centering
\caption{Tests Fonctionnels - Réservation de Transport}
\label{tab:test_reservation_transport}
\begin{tabular}{|l|p{4cm}|p{5cm}|l|}
\hline
\textbf{ID} & \textbf{Action} & \textbf{Résultat attendu} & \textbf{Résultat obtenu} \\ \hline
...
\begin{table}[H]
\centering
\caption{Tests Fonctionnels - Recherche d'Hébergements}
\label{tab:test_hebergements}
\begin{tabular}{|l|p{4cm}|p{6cm}|l|}
\hline
...
\begin{table}[H]
\centering
\caption{Tests Fonctionnels - Paiement}
\label{tab:test_paiement}
\begin{tabular}{|l|p{4cm}|p{6cm}|l|}
\hline
\textbf{ID} & \textbf{Action} & \textbf{Résultat attendu} & \textbf{Résultat obtenu} \\ \hline
...
\begin{table}[H]
\centering
\caption{Tests Fonctionnels - Avis et Notations}
\label{tab:test_avis_notations}
\begin{tabular}{|l|p{4cm}|p{6cm}|l|}
\hline
\textbf{ID} & \textbf{Action} & \textbf{Résultat attendu} & \textbf{Résultat obtenu} \\ \hline
...
\label{tab:test_securite}
\begin{tabular}{|p{8cm}|l|}
\hline
\textbf{Test} & \textbf{Résultat} \\ \hline
Protection par JWT & Conforme \\ \hline
